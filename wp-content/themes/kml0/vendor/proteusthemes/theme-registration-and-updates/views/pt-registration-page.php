...
<div class="wrap  about-wrap  pt-tr">
	...
	<div id="pt-instructions" class="pt-tr__instructions-container">
		...
		<ol>
			<li><?php printf( esc_html__( 'Go to our %1$sProteusThemes shop page%2$s and login,', 'pt-tr' ), '<a href="https://www.proteusthemes.com/account/" target="_blank">', '</a>' ); ?></li>
			<li><?php printf( esc_html__( 'You will see a list of all purchase orders that you made on our shop. In the "Details" column, you will see "View Details and Downloads" links, find the correct purchase order, that contains the theme you want to register (%s in your case),', 'pt-tr' ), '<b>' . esc_html( $this->theme_args['item_name'] ) . '</b>' ); ?></li>
			<li><?php printf( esc_html__( 'On the detail purchase order page, you will see a "License Keys" section. Copy the %s license code.', 'pt-tr' ), '<b>' . esc_html( $this->theme_args['item_name'] ) . '</b>' ); ?></li>
		</ol>
		<p>
			<?php printf( esc_html__( 'Note: if you have the Elite club or the Theme club package, don\'t use the "Elite club" or "Theme club" license codes, because they do not work, use the %s specific license code.', 'pt-tr' ), '<b>' . esc_html( $this->theme_args['item_name'] ) . '</b>' ); ?>
		</p>
	</div>
