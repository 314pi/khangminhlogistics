...
class CargoPress_Customize_Frontent {
	...
	*/
	...
	public static function get_customizer_colors_css() {
		...
		$out .= strip_tags( apply_filters( 'pt/cargopress/cached_css', $cached_css ) );
	...
	public static function get_main_title_background_gradient_css() {
		...
		if ( $main_title_background_settings['is_gradient'] ) {
			$output_gradient_css = sprintf(
				...
				esc_html( $main_title_background_settings['start_color'] ),
				esc_html( $main_title_background_settings['stop_color'] ),
				absint( $main_title_background_settings['gradient_angle'] )
			);
		}
		else {
			$output_gradient_css = sprintf( 'background-color: %1$s', esc_html( $main_title_background_settings['start_color'] ) );
		}
